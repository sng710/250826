...
@media(max-width:420px){
  ...
  .council-corner{width:54px;height:54px;left:14px;}
}
...
/* PATCH 87 - explicit mobile media carousel: one complete item at a time */
.media-v2-carousel-controls{display:none;}
@media(max-width:820px){
  .media-v2-mobile-note{display:none !important;}
  .unified-media-v2-section .media-v2-grid,
  .unified-media-v2-section .media-v2-grid[data-media-count="1"],
  .unified-media-v2-section .media-v2-grid[data-media-count="2"],
  .unified-media-v2-section .media-v2-grid[data-media-count="3"],
  ...
  .unified-media-v2-section .media-v2-item{
  ...
  .unified-media-v2-section .media-v2-item.is-carousel-active{
    display:flex !important;
  }
  ...
  .unified-media-v2-section .media-v2-facebook.is-landscape .media-v2-facebook-shell{
    width:100% !important;
    max-width:100% !important;
  }
  .unified-media-v2-section .media-v2-facebook.is-square .media-v2-facebook-shell{
    width:min(100%,320px) !important;
    max-width:320px !important;
    margin-inline:auto !important;
  }
  .unified-media-v2-section .media-v2-facebook.is-portrait .media-v2-facebook-shell{
    width:min(100%,260px) !important;
    max-width:260px !important;
    margin-inline:auto !important;
  }
  .unified-media-v2-section .media-v2-instagram-card{
    width:min(100%,340px) !important;
    margin-inline:auto !important;
  }
  .media-v2-carousel-controls{
    display:flex;
    align-items:center;
    justify-content:center;
    gap:10px;
  ...
  .media-v2-carousel-btn{
    ...
    place-items:center;
    padding:0;
    border:1px solid rgba(151,205,221,.44);
    border-radius:50%;
    background:rgba(15,35,60,.72);
    color:#fff;
    font-size:1.5rem;
    line-height:1;
    cursor:pointer;
  }
  .media-v2-carousel-btn:disabled{opacity:.34;cursor:default;}
  .media-v2-carousel-status{
    min-width:52px;
    color:#e9f3f6;
    font-size:.9rem;
    font-weight:700;
  ...
  .media-v2-carousel-dot{
    width:8px;
    height:8px;
    padding:0;
    border:0;
    border-radius:50%;
    background:rgba(221,237,243,.32);
    cursor:pointer;
  }
  .media-v2-carousel-dot.is-active{
    width:22px;
    border-radius:999px;
    background:#8ed7ec;
  }
...
<main class="person-main" id="mainContent">
  ...
  const initMobileMediaCarousels = () => {
    document.querySelectorAll('.unified-media-v2-section').forEach((section) => {
      const grid = section.querySelector('.media-v2-grid');
      const items = grid ? [...grid.querySelectorAll(':scope > .media-v2-item')] : [];
      const controls = section.querySelector('.media-v2-carousel-controls');
      if (!grid || !items.length) return;
      let index = 0;
      items.forEach((item, i) => item.classList.toggle('is-carousel-active', i === 0));
      if (!controls || items.length < 2) return;
      const prev = controls.querySelector('.media-v2-carousel-prev');
      const next = controls.querySelector('.media-v2-carousel-next');
      const status = controls.querySelector('.media-v2-carousel-status');
      ...
      const update = (nextIndex, focusMedia = false) => {
        ...
        if (focusMedia && window.matchMedia('(max-width: 820px)').matches) {
          grid.scrollIntoView({block:'nearest', behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth'});
        }
      };
      prev?.addEventListener('click', () => update(index - 1, true));
      next?.addEventListener('click', () => update(index + 1, true));
      dots.forEach((dot, i) => dot.addEventListener('click', () => update(i, true)));
      update(0);
    });
  };
